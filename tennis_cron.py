...
date into the booking window). Books a single, hardcoded slot:
    ...
    Court 6, 7:00-8:00 PM, on a target date.

Defaults to "today + 2 days" — RIOC's official rule is reservations open at
most two days in advance. (The window endpoint sometimes reports today+3, but
that's out of policy and gets auto-canceled; rioc.get_window clamps it.)
Override via --target-date; an out-of-window date is rejected before submit.
...
Examples:
    ...
    python tennis_cron.py --court 3 --start 18:00
"""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--target-date", type=date.fromisoformat,
                    default=date.today() + timedelta(days=rioc.MAX_ADVANCE_DAYS),
                    help="YYYY-MM-DD; default = today + 2 days (official advance limit)")
    ...
    ap.add_argument("--duration", type=int, default=60,
                    help="duration minutes; default 60 (RIOC allows one-hour permits only)")
